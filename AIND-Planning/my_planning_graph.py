...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            pgna = PgNode_a(a)
            if pgna.prenodes.issubset(self.s_levels[level]):              
                
                self.a_levels[level].add(pgna)
                
                for pgns_pre in pgna.prenodes:
                    for pgns in self.s_levels[level]:
                        if(pgns.__eq__(pgns_pre)):
                            pgna.parents.add(pgns)
    ...
    def add_literal_level(self, level):
        ...
        for pgna in self.a_levels[level-1]:
            pgns_set = pgna.effnodes            
            
            
            for pgns in pgns_set:                
                self.s_levels[level].add(pgns)
                pgna.children.add(pgns)
                pgns.parents.add(pgna)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len(list(set(a1.effect_add).intersection(a2.effect_rem))) > 0 \
        or len(list(set(a1.effect_rem).intersection(a2.effect_add))) > 0 :
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len(list(set(a1.effect_add).intersection(a2.precond_neg))) > 0 \
        or len(list(set(a1.effect_rem).intersection(a2.precond_pos))) > 0 \
        or len(list(set(a2.effect_add).intersection(a1.precond_neg))) > 0 \
        or len(list(set(a2.effect_rem).intersection(a1.precond_pos))) > 0:
            return True
        else:
            return False
